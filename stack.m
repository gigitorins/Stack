...
DU[expr1_,expr2_] := Module[ {temp, counterDU = 0, maxciclenumber, maxciclenumberTOT},
...
IndiceBlock[x_, y_] := Module[ {temp=1, list1, list2},
    ...
    list1 = x;
    list2 = y;
	Map[ (If[ MatchQ[#, list2[[temp]] ], temp++ ]; )& , list1];
	temp
];
...
Fitness[individuo_] := Module[ {temp, voto=1, listheads},
    ...
    If[ Depth[temp] > 8 ||  Length[listasoluzioni] > 0,
        ...
        voto = 1000., (* Penalizzo individui troppo lunghi, in questo modo sto già ottimizzando la soluzione e sto evitando loop *)

        (************************ Fitness ************************)
        corrstacks = {};
        voto += 500. * Length [Union[listheads] ] * Length[listastack]; (* Premio un patrimonio genetico vario *)
        ...
        voto += 2000. * Total[corrstacks] / (Max[corrstacks] - Min[corrstacks] + 1);
...
runs := Module[ {numrun=1000, listrun, timesrun={}, generations={}},

    testrun = Table[    listasoluzioni = {};    data = {};
                        Print["//------ RUN ",ll," ------//"];
                        listrun = AbsoluteTiming[ run ];
                        filename = "Runs_6/run"<>ToString[ll]<>".dat";
                        Export[ filename, data , "Table",  "FieldSeparators" -> " ", OverwriteTarget -> True];
                        listrun,
                        {ll,1,numrun} ];
        
    filename = "Runs_6/a_Soluzioni.dat";
    Export[ filename, soluzionitotali , "Table",  "FieldSeparators" -> " ", OverwriteTarget -> True];
